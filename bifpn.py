...
def ReSize(input_tensor, mode='downsample', target_shape=None):
    '''
    ...
    target_shape : Optional tuple (height, width) to resize to specific dimensions
    ...
    if target_shape is not None:
        op = Resizing(height=target_shape[0], width=target_shape[1], interpolation='bilinear')(input_tensor)
...
def SE_block(input_tensor, ratio=8, activation=tf.nn.swish, Name=None):
    input_tensor = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                                   name='se_dw_{}'.format(Name))(input_tensor)
...
def BiFPN(p2, p3, p4, p5, p6, p7, bifpn_layer, Dropout_Rate=0.3, Use_Dropout=False):
    # Get shapes of all input tensors for potential resizing
    _, p2_h, p2_w, _ = int_shape(p2)
    _, p3_h, p3_w, _ = int_shape(p3)
    _, p4_h, p4_w, _ = int_shape(p4)
    _, p5_h, p5_w, _ = int_shape(p5)
    _, p6_h, p6_w, _ = int_shape(p6)
    _, p7_h, p7_w, _ = int_shape(p7)
    ...
    p6_td = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                            name='dw_p6td_bifpn_{}'.format(bifpn_layer))(p6_td)
    ...
    p5_td = add([SE_block(p5, Name='se_td_p5_{}'.format(bifpn_layer)),
                 SE_block(ReSize(p6_td, mode='upsample', target_shape=(p5_h, p5_w)), Name='se_td_p6td_{}'.format(bifpn_layer))],
                name='add_p5p6td_{}'.format(bifpn_layer))

    p5_td = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                            name='dw_p5td_bifpn_{}'.format(bifpn_layer))(p5_td)
    ...
    p4_td = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                            name='dw_p4td_bifpn_{}'.format(bifpn_layer))(p4_td)
    ...
    p3_td = add([SE_block(p3, Name='se_td_p3_{}'.format(bifpn_layer)),
                 SE_block(ReSize(p4_td, mode='upsample', target_shape=(p3_h, p3_w)), Name='se_td_p4td_{}'.format(bifpn_layer))],
                name='add_p3p4td_{}'.format(bifpn_layer))

    p3_td = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                            name='dw_p3td_bifpn_{}'.format(bifpn_layer))(p3_td)
    ...
    p2_out = add([SE_block(p2, Name='se_td_p2_{}'.format(bifpn_layer)),
                  SE_block(ReSize(p3_td, mode='upsample', target_shape=(p2_h, p2_w)), Name='se_td_p3td_{}'.format(bifpn_layer))],
                 name='add_p2p3td_{}'.format(bifpn_layer))

    p2_out = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p2out_bifpn_{}'.format(bifpn_layer))(p2_out)
    ...
    p3_out = add([SE_block(p3, Name='se_bu_p3_{}'.format(bifpn_layer)),
                  ...
                  SE_block(ReSize(p2_out, mode='downsample', target_shape=(p3_h, p3_w)), Name='se_bu_p2out_{}'.format(bifpn_layer))],
                 name='add_p3p3tdp2out_{}'.format(bifpn_layer))

    p3_out = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p3out_bifpn_{}'.format(bifpn_layer))(p3_out)
    ...
    p4_out = add([SE_block(p4, Name='se_bu_p4_{}'.format(bifpn_layer)),
                  ...
                  SE_block(ReSize(p3_out, mode='downsample', target_shape=(p4_h, p4_w)), Name='se_bu_p3out_{}'.format(bifpn_layer))],
                 name='add_p4p4tdp3out_{}'.format(bifpn_layer))

    p4_out = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p4out_bifpn_{}'.format(bifpn_layer))(p4_out)
    ...
    p5_out = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p5out_bifpn_{}'.format(bifpn_layer))(p5_out)
    ...
    p6_out = add([SE_block(p6, Name='se_bu_p6_{}'.format(bifpn_layer)),
                  ...
                  SE_block(ReSize(p5_out, mode='downsample', target_shape=(p6_h, p6_w)), Name='se_bu_p5out_{}'.format(bifpn_layer))],
                 name='add_p6p6tdp5out_{}'.format(bifpn_layer))

    p6_out = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p6out_bifpn_{}'.format(bifpn_layer))(p6_out)
    ...
    p7_out = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p7out_bifpn_{}'.format(bifpn_layer))(p7_out)
...
def OP_BiFPN(p2, p3, p4, p5, p6, p7, bifpn_layer, Dropout_Rate=0.3, Use_Dropout=False):
    ...
    p6_seg = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p6seg_bifpn_{}'.format(bifpn_layer))(p6_seg)
    ...
    p5_seg = add([SE_block(p5, Name='se_seg_p5_{}'.format(bifpn_layer)),
                  SE_block(ReSize(p6_seg, mode='upsample', target_shape=(p5_h, p5_w)), Name='se_seg_p6seg_{}'.format(bifpn_layer))],
                 name='add_p5p6seg_{}'.format(bifpn_layer))

    p5_seg = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p5seg_bifpn_{}'.format(bifpn_layer))(p5_seg)
    ...
    p4_seg = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p4seg_bifpn_{}'.format(bifpn_layer))(p4_seg)
    ...
    p3_seg = add([SE_block(p3, Name='se_seg_p3_{}'.format(bifpn_layer)),
                  SE_block(ReSize(p4_seg, mode='upsample', target_shape=(p3_h, p3_w)), Name='se_seg_p4seg_{}'.format(bifpn_layer))],
                 name='add_p3p4seg_{}'.format(bifpn_layer))

    p3_seg = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p3seg_bifpn_{}'.format(bifpn_layer))(p3_seg)
    ...
    p2_seg = add([SE_block(p2, Name='se_seg_p2_{}'.format(bifpn_layer)),
                  SE_block(ReSize(p3_seg, mode='upsample', target_shape=(p2_h, p2_w)), Name='se_seg_p3seg_{}'.format(bifpn_layer))],
                 name='add_p2p3seg_{}'.format(bifpn_layer))

    p2_seg = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                             name='dw_p2segout_bifpn_{}'.format(bifpn_layer))(p2_seg)
    ...
    p6_inst = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                              name='dw_p6inst_bifpn_{}'.format(bifpn_layer))(p6_inst)
    ...
    p5_inst = add([SE_block(ReSize(p6, mode='upsample', target_shape=(p5_h, p5_w)), Name='se_inst_p6_{}'.format(bifpn_layer)),
                   SE_block(ReSize(p6_inst, mode='upsample', target_shape=(p5_h, p5_w)), Name='se_inst_p6inst_{}'.format(bifpn_layer)),
                   SE_block(p5_seg, Name='se_p5seg_inst_{}'.format(bifpn_layer))],
                  name='add_p5segp6p6inst_{}'.format(bifpn_layer))

    p5_inst = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                              name='dw_p5inst_bifpn_{}'.format(bifpn_layer))(p5_inst)
    ...
    p4_inst = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                              name='dw_p4inst_bifpn_{}'.format(bifpn_layer))(p4_inst)
    ...
    p3_inst = add([SE_block(ReSize(p4, mode='upsample', target_shape=(p3_h, p3_w)), Name='se_inst_p4_{}'.format(bifpn_layer)),
                   SE_block(ReSize(p4_inst, mode='upsample', target_shape=(p3_h, p3_w)), Name='se_inst_p4inst_{}'.format(bifpn_layer)),
                   SE_block(p3_seg, Name='se_p3seg_inst_{}'.format(bifpn_layer))],
                  name='add_p3segp4p4inst_{}'.format(bifpn_layer))

    p3_inst = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                              name='dw_p3inst_bifpn_{}'.format(bifpn_layer))(p3_inst)
    ...
    p2_inst = add([SE_block(ReSize(p3, mode='upsample', target_shape=(p2_h, p2_w)), Name='se_inst_p3_{}'.format(bifpn_layer)),
                   SE_block(ReSize(p3_inst, mode='upsample', target_shape=(p2_h, p2_w)), Name='se_inst_p3inst_{}'.format(bifpn_layer)),
                   SE_block(p2_seg, Name='se_p2seg_inst_{}'.format(bifpn_layer))],
                  name='add_p2segp3p3inst_{}'.format(bifpn_layer))

    p2_inst = DepthwiseConv2D((3, 3), padding='same', depthwise_regularizer=L2(4e-5),
                              name='dw_p2instout_bifpn_{}'.format(bifpn_layer))(p2_inst)
