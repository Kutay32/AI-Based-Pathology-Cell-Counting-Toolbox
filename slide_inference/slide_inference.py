...
# Import custom model loader to handle DepthwiseConv2D with 'groups' parameter
import sys
...
try:
    ...
    try:
        ...
        warnings.warn("Could not import model_h5_wrapper. Falling back to standard model loading.")
...
try:
    # Try to use the custom model loader to handle DepthwiseConv2D with 'groups' parameter
    model = load_model_h5(model_path)
    print('=> Model loaded successfully using custom loader')
except (NameError, ImportError) as e:
    # Fall back to standard loader if custom loader is not available
    print('=> Custom loader not available, falling back to standard loader')
    model = tf.keras.models.load_model(filepath=model_path, compile=False)
    print('=> Model loaded successfully')
except Exception as e:
    print(f'=> Error loading model with custom loader: {str(e)}')
